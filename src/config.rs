    ...
    for (orig_lineno, line) in &lines {
        ...
        if l.is_empty() {
            ...
            if let Some(ref mut bb) = brace_block {
                if bb.in_else {
                    bb.else_lines.push(String::new());
                } else {
                    bb.true_lines.push(String::new());
                }
            }
            continue;
        }
...
fn parse_set_option(app: &mut AppState, line: &str) {
    ...
    if parts.len() < 2 {
        warn_config(app, "set-option requires an option name");
        return;
    }
...
pub fn parse_option_value(app: &mut AppState, rest: &str, _is_global: bool) {
                ...
                "boolean" => {
                    ...
                    if !ok {
                        warn_config(
                            app,
                            format!(
                                "invalid value '{}' for option '{}' (expected on/off)",
                                v, key
                            ),
                        );
                    }
    ...
    match key {
        ...
        "scroll-enter-copy-mode" => {
            app.scroll_enter_copy_mode = matches!(value, "on" | "true" | "1")
        }
        ...
        "cursor-blink" => {
            ...
            let _ = std::io::Write::write_all(
                &mut std::io::stdout(),
                if on { b"\x1b[?12h" } else { b"\x1b[?12l" },
            );
        ...
        "pane-border-hover-style" => {
            app.pane_border_hover_style = value.to_string();
        }
...
fn parse_tmux_entry_script(app: &mut AppState, path: &std::path::Path) {
    ...
    for line in content.lines() {
        ...
        } else if expanded.starts_with("set-option ")
            || expanded.starts_with("set ")
