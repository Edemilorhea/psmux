...
impl Write for FailingWriter {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        if self.fail_flush {
            ...
            Err(io::Error::new(io::ErrorKind::BrokenPipe, "write channel closed"))
        }
    }

    fn flush(&mut self) -> io::Result<()> {
        Err(io::Error::new(io::ErrorKind::BrokenPipe, "flush channel closed"))
    }
}

#[test]
fn write_input_writes_and_flushes_bytes() {
    let mut writer = Vec::new();

    write_input(&mut writer, b"\x1b").expect("healthy writer accepts input");

    assert_eq!(writer, b"\x1b");
}

#[test]
fn write_input_returns_write_failure() {
    let mut writer = FailingWriter { fail_flush: false };

    let error = write_input(&mut writer, b"\x1b").expect_err("closed writer must be observable");

    assert_eq!(error.kind(), io::ErrorKind::BrokenPipe);
    assert_eq!(error.to_string(), "write channel closed");
}

#[test]
fn write_input_returns_flush_failure() {
    let mut writer = FailingWriter { fail_flush: true };

    let error = write_input(&mut writer, b"\x03").expect_err("flush failure must be observable");

    assert_eq!(error.kind(), io::ErrorKind::BrokenPipe);
    assert_eq!(error.to_string(), "flush channel closed");
}
...
fn cursor_keys_keep_csi_when_app_cursor_off() {
    ...
    for csi in [
        ...
        assert_eq!(
            csi_cursor_to_ss3(csi, false),
            None,
            "app-cursor off must keep CSI"
        );
    }
}
...
fn decckm_parser_state_drives_arrow_encoding() {
    ...
    assert!(
        ...
        "fresh screen defaults to off"
    );
    assert_eq!(
        csi_cursor_to_ss3(b"\x1b[A", term.screen().application_cursor()),
        None
    );
...
fn ctrl_c_key_event_detects_uppercase_control_c() {
    ...
    assert!(
        is_ctrl_c_key_event(&ev),
        "Ctrl+C must be recognized as interrupt key"
    );
}
...
fn ctrl_c_key_event_detects_raw_etx() {
    ...
    assert!(
        is_ctrl_c_key_event(&ev),
        "raw ETX (0x03) must be recognized as Ctrl+C"
    );
}
...
fn ctrl_c_key_event_rejects_alt_modified_c() {
    ...
    assert!(
        !is_ctrl_c_key_event(&ev),
        "Ctrl+Alt+C must not be treated as plain Ctrl+C"
    );
}
...
fn shift_enter_no_ctrl_uses_vt_not_csi() {
    ...
    assert_eq!(
        bytes, b"\x1b\r",
        "Shift+Enter (no Ctrl) on Windows must use VT encoding (ESC+CR); got {:?}",
        bytes
    );
}
...
fn alt_enter_no_ctrl_uses_vt_not_csi() {
    ...
    assert_eq!(
        bytes, b"\x1b\r",
        "Alt+Enter (no Ctrl) on Windows must use VT encoding (ESC+CR); got {:?}",
        bytes
    );
}
...
fn ctrl_enter_uses_platform_encoding() {
    ...
    assert_eq!(
        bytes, b"\n",
        "Ctrl+Enter must use LF on Windows; got {:?}",
        bytes
    );
    #[cfg(not(windows))]
    assert_eq!(
        bytes, b"\x1b[13;5~",
        "Ctrl+Enter must use CSI encoding; got {:?}",
        bytes
    );
}
...
fn bug3_double_delivery_prevention() {
    ...
    assert!(
        !shift_enter.modifiers.contains(KeyModifiers::CONTROL),
        "Shift+Enter must NOT trigger the ctrl guard (no native injection)"
    );
    assert!(
        !alt_enter.modifiers.contains(KeyModifiers::CONTROL),
        "Alt+Enter must NOT trigger the ctrl guard (no native injection)"
    );
    assert!(
        ctrl_enter.modifiers.contains(KeyModifiers::CONTROL),
