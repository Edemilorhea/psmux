...
use crate::cli::{extract_flag_value, parse_target};
use crate::control;
use crate::types::{ControlNotification, CtrlReq, LayoutKind, WaitForOp};
...
fn decode_send_command(line: &str) -> Option<(String, Vec<u8>)> {
    ...
    let any_short = |c: char| {
        args.iter().any(|a| {
            a.starts_with('-') && !a.starts_with("--") && a.chars().skip(1).any(|fc| fc == c)
        })
    };
    if any_short('X') || any_short('p') || any_short('N') || any_short('R') {
        return None;
    }
...
fn coalesce_send_commands(parts: Vec<String>) -> Vec<String> {
    ...
    for part in parts {
        match decode_send_command(&part) {
            Some((tgt, bytes)) => {
                let target_match = acc.is_empty() || acc_target.as_deref() == Some(tgt.as_str());
                if !target_match {
...
fn parse_new_pane_args(args: &[&str]) -> ParsedNewPane {
    ...
    let (mut x, mut y, mut w, mut h): (Option<u16>, Option<u16>, Option<u16>, Option<u16>) =
        (None, None, None, None);
        ...
        match args[i] {
            ...
            "-B" => {
                if let Some(v) = args.get(i + 1) {
                    border = v.trim_matches('"').to_string();
                    skip.insert(i);
                    skip.insert(i + 1);
                    i += 1;
                }
...
pub(crate) fn handle_connection(
    ...
    if control_echo || control_noecho {
        ...
        let notif_thread = std::thread::spawn(move || {
            while let Ok(notif) = notif_rx.recv() {
                ...
                if writeln!(ws, "{}", formatted).is_err() {
                    break;
                }
                if ws.flush().is_err() {
                    break;
        ...
        loop {
            let trimmed_owned: String = if let Some(s) = pending.pop_front() {
                ...
                let stripped =
                    trimmed_raw.trim_start_matches(|c: char| (c as u32) < 0x20 && c != '\t');
                    ...
                    if cmd_args[i] == "-t" {
                        if let Some(v) = cmd_args.get(i + 1) {
                            ...
                            if pt.window.is_some() {
                                ctrl_target_win = pt.window;
                                ctrl_target_win_is_id = pt.window_is_id;
                                ctrl_target_win_name = None;
                            } else if pt.window_name.is_some() {
                                ctrl_target_win_name = pt.window_name;
            ...
            match response_result {
                Some(Ok(response)) => {
                    ...
                    let (is_error, body) =
                        if let Some(stripped) = response.strip_prefix("\u{0001}ERR\u{0001}") {
        ...
        let _ = tx.send(CtrlReq::ControlDeregister {
            client_id: ctrl_client_id,
        });
        drop(notif_thread);
        return;
    ...
    loop {
            ...
            if args[i] == "-t" {
                if let Some(v) = args.get(i + 1) {
                    ...
                    if pt.window.is_some() {
                        ...
                        target_win_is_id = pt.window_is_id;
                        target_win_name = None;
                    } else if pt.window_name.is_some() {
                        target_win_name = pt.window_name;
                        target_win = None;
        ...
        let skip_pane_focus = matches!(cmd, "display-message" | "display" | "swap-pane" | "swapp")
            || skip_target_focus;
        ...
        match cmd {
            "new-window" | "neww" => {
                ...
                let format_str: Option<String> =
                    extract_flag_value(&args, "-F").map(|s| s.trim_matches('"').to_string());
                let title: Option<String> =
                    extract_flag_value(&args, "-T").map(|s| s.trim_matches('"').to_string());
                ...
                let cmd_str: Option<String> = args
                    ...
                    .find(|a| {
                        !a.starts_with('-')
                            ...
                            && args.windows(2).all(|w| !(w[0] == "-T" && w[1] == **a))
                            && !args
                                .iter()
                                .any(|f| f.starts_with("-F") && f.len() > 2 && &f[2..] == **a)
                    })
                    .map(|s| s.trim_matches('"').to_string());
                if print_info {
                    let (rtx, rrx) = mpsc::channel::<String>();
                    let _ = tx.send(CtrlReq::NewWindowPrint(
                        cmd_str, name, detached, start_dir, format_str, rtx, title, empty,
                    ));
                    ...
                    let _ = tx.send(CtrlReq::NewWindow(
                        cmd_str, name, detached, start_dir, title, empty,
                    ));
                }
            }
            "split-window" | "splitw" | "split-pane" | "splitp" => {
                ...
                let split_size: Option<(u16, bool)> = args
                    .windows(2)
                    .find(|w| w[0] == "-p")
                ...
                if print_info {
                    ...
                    let _ = tx.send(CtrlReq::SplitWindowPrint(
                        kind, cmd_str, detached, start_dir, split_size, format_str, rtx, title,
                    ));
                    if let Ok(text) = rrx.recv_timeout(Duration::from_millis(2000)) {
                        let _ = write!(write_stream, "{}\n", text);
                        let _ = write_stream.flush();
                    ...
                    let _ = tx.send(CtrlReq::SplitWindow(
                        kind, cmd_str, detached, start_dir, split_size, rtx, title,
                    ));
            ...
            "capture-pane" | "capturep" => {
                ...
                if let Ok(mut text) = rrx.recv() {
                    ...
                    if print_stdout {
                        ...
                        if persistent {
                            let _ =
                                tx.send(CtrlReq::ShowTextPopup("capture-pane".to_string(), text));
                        } else {
            ...
            "zoom-pane" | "resize-pane" | "resizep" if args.iter().any(|a| *a == "-Z") => {
            ...
            "focus-pane" => {
                if let Some(pid) = args.get(0).and_then(|s| s.parse::<usize>().ok()) {
                    let _ = tx.send(CtrlReq::FocusPaneCmd(pid));
                }
            }
            "focus-window" => {
                if let Some(wid) = args.get(0).and_then(|s| s.parse::<usize>().ok()) {
            ...
            "mouse-down" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseDown(client_id, x, y));
                    }
                }
            }
            "mouse-down-right" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseDownRight(client_id, x, y));
                    }
                }
            }
            "mouse-down-middle" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseDownMiddle(client_id, x, y));
                    }
                }
            }
            "mouse-drag" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseDrag(client_id, x, y));
                    }
                }
            }
            "mouse-up" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseUp(client_id, x, y));
                    }
                }
            }
            "mouse-up-right" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseUpRight(client_id, x, y));
                    }
                }
            }
            "mouse-up-middle" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseUpMiddle(client_id, x, y));
                    }
                }
            }
            "mouse-move" => {
                if args.len() >= 2 {
                    if let (Ok(x), Ok(y)) = (args[0].parse::<u16>(), args[1].parse::<u16>()) {
                        let _ = tx.send(CtrlReq::MouseMove(client_id, x, y));
                    }
                }
            }
            "scroll-up" => {
            ...
            "pane-mouse" => {
                ...
                if args.len() >= 5 {
                    if let (Ok(pane_id), Ok(button), Ok(col), Ok(row)) = (
                        ...
                        let press = args[4] != "m";
                        let _ = tx.send(CtrlReq::PaneMouse(
                            client_id, pane_id, button, col, row, press,
                        ));
                    }
                }
            }
            ...
            "split-sizes" => {
                ...
                if args.len() >= 2 {
                    ...
                    let sizes: Vec<u16> =
                        args[1].split(',').filter_map(|s| s.parse().ok()).collect();
                    if sizes.len() >= 2 {
            ...
            "list-windows" | "lsw" => {
                ...
                if let Some(fmt_str) = fmt {
                    ...
                    if let Ok(text) = rrx.recv() {
                        if persistent {
                            let _ =
                                tx.send(CtrlReq::ShowTextPopup("list-windows".to_string(), text));
                        } else {
                            let _ = write!(write_stream, "{}\n", text);
                            let _ = write_stream.flush();
                        }
            ...
            "list-tree" => {
                ...
                if let Ok(text) = rrx.recv() {
                    if persistent {
                        let _ = tx.send(CtrlReq::ShowTextPopup("list-tree".to_string(), text));
                    } else {
                        let _ = write!(write_stream, "{}\n", text);
                        let _ = write_stream.flush();
                    }
            ...
            "send-keys" | "send" => {
                ...
                // -f / --force-signal: route C-c through a genuine Windows
                // CTRL_BREAK_EVENT. Intended for explicit kill bindings like
                // `bind -n C-F12 send-keys -f C-c` in psmux.conf.
                ...
                if let Some(n_pos) = args
                    .iter()
                    .position(|a| a.starts_with('-') && !a.starts_with("--") && a.ends_with('N'))
                {
                ...
                if let Some(ref tgt) = raw_target {
                    ...
                    if let Ok(port_str) = std::fs::read_to_string(&port_path) {
                        if let Ok(port) = port_str.trim().parse::<u16>() {
                            ...
                            let _ =
                                crate::session::send_control_to_port(port, "kill-session\n", &key);
                            handled = true;
                        }
                    }
                    if !handled {
            ...
            "swap-pane" | "swapp" => {
                ...
                } else if let Some((p, is_id)) = raw_t
                    .map(parse_target)
                    .and_then(|pt| pt.pane.map(|p| (p, pt.pane_is_id)))
                {
                    let _ = tx.send(CtrlReq::SwapPaneTarget(p, is_id));
                } else {
                    let dir = if args.iter().any(|a| *a == "-U") {
                        "U"
                    } else if args.iter().any(|a| *a == "-L") {
                        "L"
                    } else if args.iter().any(|a| *a == "-R") {
            ...
            "paste-buffer" | "pasteb" => {
                let buf_name: Option<String> = args
                    .windows(2)
                    .find(|w| w[0] == "-b")
                    .map(|w| w[1].to_string());
            ...
            "display-message" | "display" => {
                ...
                if pane_is_id {
                    if let Some(pid) = target_pane {
                        let _ = tx.send(CtrlReq::DisplayMessageById(
                            rtx,
                            fmt,
                            pid,
                            !print_stdout,
                ...
                if let Ok(text) = rrx.recv() {
                    if print_stdout {
                        if persistent {
                            let _ = tx
                                .send(CtrlReq::ShowTextPopup("display-message".to_string(), text));
                        } else {
            ...
            "set-option" | "set" | "set-window-option" | "setw" => {
                ...
                let t_targets: std::collections::HashSet<&str> = args
                    .windows(2)
                ...
                let non_flag_args: Vec<&str> = args
                    .iter()
                    .filter(|a| {
                        (!a.starts_with('-') || a.starts_with('@')) && !t_targets.contains(*a)
                    })
                    .copied()
                    .collect();
                if has_u {
                ...
                if has_v && opt_name.is_some() || (opt_name.is_some() && !has_q) {
                    ...
                    if let Ok(text) = rrx.recv() {
                        ...
                        let values_only: String = text
                            ...
                            .collect::<Vec<_>>()
                            .join("\n");
                        let output = if values_only.is_empty() {
                            String::new()
                        } else {
                            format!("{}\n", values_only)
                        };
                        if persistent {
                            let _ =
                                tx.send(CtrlReq::ShowTextPopup("show-options".to_string(), output));
                        } else {
                            let _ = write_stream.write_all(output.as_bytes());
                            let _ = write_stream.flush();
                        }
                    ...
                    if window_scope {
                        ...
                        if let Ok(mut text) = rrx.recv() {
                            ...
                            if persistent {
                                let _ = tx
                                    .send(CtrlReq::ShowTextPopup("show-options".to_string(), text));
                            } else {
                                let _ = write!(write_stream, "{}\n", text);
                                let _ = write_stream.flush();
                            }
            ...
            "show-environment" | "showenv" => {
                ...
                if let Ok(text) = rrx.recv() {
                    if persistent {
                        let _ =
                            tx.send(CtrlReq::ShowTextPopup("show-environment".to_string(), text));
                    } else {
                        let _ = write!(write_stream, "{}\n", text);
                        let _ = write_stream.flush();
                    }
            ...
            "new-pane" | "newp" => {
                ...
                if p.print {
                    let (rtx, rrx) = mpsc::channel::<String>();
                    let _ = tx.send(CtrlReq::NewFloat {
                        command: p.command,
                        x: p.x,
                        y: p.y,
                        w: p.w,
            ...
            "detach-client" | "detach" => {
                ...
                let target_cid_numeric: Option<u64> = target_str
                    .as_ref()
                ...
                if detach_session {
                    ...
                    if cid == client_id {
                        if kill_parent {
                            let _ = crate::types::send_directive_to_client(
                                client_id,
                                "DETACH-KILL-PARENT",
                            );
                        }
                    ...
                    if kill_parent {
                        let _ =
                            crate::types::send_directive_to_client(client_id, "DETACH-KILL-PARENT");
                    }
                    let _ = tx.send(CtrlReq::ClientDetach(client_id));
                    attached_sent = false;
                }
            ...
            "list-sessions" | "ls" => {
                ...
                if let Some(fmt_str) = fmt {
                    ...
                    if let Ok(text) = rrx.recv() {
                        if persistent {
                            let _ =
                                tx.send(CtrlReq::ShowTextPopup("list-sessions".to_string(), text));
                        } else {
                            let _ = write!(write_stream, "{}\n", text);
                            let _ = write_stream.flush();
                        }
            ...
            "new-session" | "new" => {
                    ...
                    .map(|w| w[1].to_string())
                {
                        ...
                        while i < args.len() {
                            match args[i] {
                                "-s" => {
                                    i += 1;
                                    if i < args.len() {
                                        sess_name = Some(args[i].trim_matches('"').to_string());
                                    }
                                }
                                "-n" => {
                                    i += 1;
                                    if i < args.len() {
                                        window_name = Some(args[i].trim_matches('"').to_string());
                                    }
                                }
                                "-c" => {
                                    i += 1;
                                    if i < args.len() {
                                        start_dir = Some(args[i].trim_matches('"').to_string());
                                    }
                                }
                                "-x" => {
                                    i += 1;
                                    if i < args.len() {
                                        init_width = Some(args[i].to_string());
                                    }
                                }
                                "-y" => {
                                    i += 1;
                                    if i < args.len() {
                                        init_height = Some(args[i].to_string());
                                    }
                                }
                                "-e" => {
                                    i += 1;
                    ...
                    if let Some(ref err) = env_parse_err {
                        ...
                        let name =
                            sess_name.unwrap_or_else(|| crate::session::next_session_name(None));
                        ...
                        if already_exists {
                            if persistent {
                                let _ = tx.send(CtrlReq::StatusMessage(format!(
    ...
    match cmd {
        ...
        "new-pane" | "newp" => {
            ...
            if p.print {
                ...
                if let Ok(text) = rrx.recv_timeout(Duration::from_secs(2)) {
                    let _ = resp_tx.send(text);
                }
                true
            } else {
        ...
        "capture-pane" | "capturep" => {
            let start = args.windows(2).find(|w| w[0] == "-S").and_then(|w| {
                if w[1] == "-" {
                    Some(i32::MIN)
                } else {
                    w[1].parse::<i32>().ok()
        ...
        "set-option" | "set" | "set-window-option" | "setw" => {
            ...
            let t_vals2: std::collections::HashSet<&str> = args
                .windows(2)
        ...
        | "show-window-option" => {
            ...
            let window_scope2 =
                matches!(cmd, "show-window-options" | "showw" | "show-window-option")
                    || combined_has2('w');
            let opt_name = args
                .iter()
                .filter(|a| !a.starts_with('-'))
        ...
        "swap-pane" | "swapp" => {
            ...
            if let (Some((sv, sid)), Some((dv, did))) = (src, dst) {
                ...
                if let Some((p, is_id)) = resolved {
                    ...
                    let direction = if args.iter().any(|a| *a == "-U") {
                        "U".to_string()
                    } else if args.iter().any(|a| *a == "-L") {
                        "L".to_string()
                    } else if args.iter().any(|a| *a == "-R") {
