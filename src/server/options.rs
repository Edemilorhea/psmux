...
pub(crate) fn apply_set_option(app: &mut AppState, option: &str, value: &str, _quiet: bool) {
    match option {
        ...
        "pwsh-mouse-selection" => {
            app.pwsh_mouse_selection = matches!(value, "on" | "true" | "1");
        }
        "mouse-selection" => {
            app.mouse_selection = matches!(value, "on" | "true" | "1");
        }
        "paste-detection" => {
            app.paste_detection = matches!(value, "on" | "true" | "1");
        }
        "choose-tree-preview" => {
            app.choose_tree_preview = matches!(value, "on" | "true" | "1");
        }
        "prefix" => {
