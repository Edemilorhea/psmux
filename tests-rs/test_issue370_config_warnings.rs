...
fn unknown_command_warns() {
    ...
    assert!(
        warns_contain(&a, "unknown command: totally-bogus-command"),
        "expected unknown-command warning, got: {:?}",
        a.config_warnings
    );
}
...
fn typo_of_real_command_warns() {
    ...
    assert!(
        warns_contain(&a, "unknown command: bnid-key"),
        "typo'd command should warn, got: {:?}",
        a.config_warnings
    );
}
...
fn known_but_unrouted_command_does_not_warn() {
    ...
    assert!(
        !warns_contain(&a, "unknown command"),
        "known commands must not warn, got: {:?}",
        a.config_warnings
    );
}
...
fn routed_commands_do_not_warn() {
    ...
    assert!(
        ...
        "valid routed directives must not warn, got: {:?}",
        a.config_warnings
    );
}
...
fn unknown_option_warns() {
    ...
    assert!(
        warns_contain(&a, "unknown option 'this-is-not-a-real-option'"),
        "expected unknown-option warning, got: {:?}",
        a.config_warnings
    );
}
...
fn at_prefixed_user_option_never_warns() {
    ...
    crate::config::parse_config_content(
        &mut a,
        "set -g @my-plugin-setting yes\nset -g @plugin 'foo/bar'\n",
    );
    assert!(
        !warns_contain(&a, "unknown option"),
        "@-options are user/plugin options and must never warn, got: {:?}",
        a.config_warnings
    );
}
...
fn array_style_option_does_not_warn() {
    ...
    assert!(
        !warns_contain(&a, "unknown option"),
        "array-style option keys must not warn, got: {:?}",
        a.config_warnings
    );
}
...
fn known_options_do_not_warn() {
    ...
    assert!(
        a.config_warnings.is_empty(),
        "known options must not warn, got: {:?}",
        a.config_warnings
    );
}
...
fn malformed_numeric_value_warns() {
    ...
    assert!(
        warns_contain(&a, "invalid value 'notanumber' for option 'escape-time'"),
        "expected malformed-number warning, got: {:?}",
        a.config_warnings
    );
}
...
fn malformed_numeric_keeps_prior_value() {
    ...
    assert_eq!(
        a.escape_time_ms, 77,
        "bad value must not overwrite the good one"
    );
...
fn malformed_boolean_value_warns() {
    ...
    assert!(
        warns_contain(&a, "invalid value 'maybe' for option 'mouse'"),
        "expected malformed-boolean warning, got: {:?}",
        a.config_warnings
    );
}
...
fn valid_numeric_and_boolean_do_not_warn() {
    ...
    assert!(
        a.config_warnings.is_empty(),
        "valid values must not warn (incl. numeric 'status 2'), got: {:?}",
        a.config_warnings
    );
}
...
fn bare_set_option_warns() {
    ...
    assert!(
        warns_contain(&a, "set-option requires an option name"),
        "bare set must warn, got: {:?}",
        a.config_warnings
    );
}
...
fn warning_carries_line_number_when_file_known() {
    ...
    assert!(
        a.config_warnings
            .iter()
            .any(|w| w.contains("bogus-directive") && w.contains(":2:")),
        "warning should be tagged with line 2, got: {:?}",
        a.config_warnings
    );
