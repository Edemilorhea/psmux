...
/// mapped to lowercase.
use super::*;
use crate::config::{normalize_key_for_binding, parse_bind_key, parse_key_name};
...
fn parse_bind_key_stores_uppercase_binding() {
    ...
    let has_uppercase_i = app
        .key_tables
        .get("prefix")
        .map(|t| {
            t.iter()
                .any(|b| b.key == (KeyCode::Char('I'), KeyModifiers::NONE))
        })
        .unwrap_or(false);
...
fn parse_bind_key_stores_lowercase_binding_separately() {
    ...
    let has_lower = prefix
        .iter()
        .any(|b| b.key == (KeyCode::Char('i'), KeyModifiers::NONE));
    let has_upper = prefix
        .iter()
        .any(|b| b.key == (KeyCode::Char('I'), KeyModifiers::NONE));
    ...
    assert_eq!(
        prefix
            .iter()
            .filter(|b| matches!(b.key.0, KeyCode::Char('i') | KeyCode::Char('I')))
            .count(),
        2,
