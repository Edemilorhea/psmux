...
fn send_text_digit_sets_copy_count() {
    ...
    assert_eq!(
        app.copy_count,
        Some(5),
        "send-text '5' in copy mode should set copy_count=5"
    );
}
...
fn send_text_digits_accumulate() {
    ...
    assert_eq!(
        app.copy_count,
        Some(10),
        "'1' then '0' should accumulate to 10 (not line-start)"
    );
}
...
fn send_text_digits_accumulate_single_burst() {
    ...
    assert_eq!(
        app.copy_count,
        Some(10),
        "'10' in one burst should accumulate to 10"
    );
}
...
fn send_text_find_char_preserves_count_until_target() {
    ...
    assert_eq!(
        app.copy_find_char_pending,
        Some(0),
        "'f' should arm find-char forward"
    );
    assert_eq!(
        app.copy_count,
        Some(3),
        "'f' must preserve the count for the target char"
    );
}
