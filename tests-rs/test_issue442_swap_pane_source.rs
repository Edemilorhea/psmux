...
fn without_detach_the_t_pane_becomes_active() {
    ...
    assert_eq!(
        active_index(&app),
        0,
        "the -t pane now sits in the src slot (idx0)"
    );
}
...
fn active_pane_is_not_the_implicit_source() {
    ...
    assert_eq!(
        idx_of_4, 1,
        "BUG #442: the active pane must NOT be the implicit source"
    );
}
...
fn detach_keeps_focus_on_untouched_active_pane() {
    ...
    assert_eq!(
        active_index(&app),
        1,
        "id 4 never moved, so focus stays at idx1"
    );
...
fn detach_follows_active_pane_when_it_is_swapped() {
    ...
    assert_eq!(
        active_index(&app),
        3,
        "id 2 moved to idx3, focus follows it there"
    );
}
