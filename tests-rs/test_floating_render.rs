...
fn cell_char(buf: &ratatui::buffer::Buffer, x: u16, y: u16) -> char {
    ...
    buf.content[(y as usize) * w + (x as usize)]
        .symbol()
        .chars()
        .next()
        .unwrap_or(' ')
...
fn make_float(x: u16, y: u16, w: u16, h: u16, border: &str) -> FloatJson {
    ...
    let rows: Vec<RowRunsJson> = (0..h.saturating_sub(2))
        .map(|_| row_of('Z', (w.saturating_sub(2)) as usize))
        .collect();
    FloatJson {
        x,
        y,
...
fn single_border_and_content_positioned() {
    ...
    assert_eq!(
        cell_char(&buf, 5, 3),
        '┌',
        "single top-left corner at float origin"
    );
...
fn none_border_draws_content_without_box() {
    ...
    assert_eq!(
        cell_char(&buf, 4, 2),
        'Z',
        "none: content at origin, no border"
    );
    for bad in ['┌', '╔', '┏'] {
        let found = buf
            .content
            .iter()
            .any(|c| c.symbol().chars().next() == Some(bad));
