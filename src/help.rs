...
pub const ROOT_DEFAULTS: &[(&str, &str)] = &[("PageUp", "copy-mode -u")];
...
const CLI_COMMANDS: &[(&str, &str, &str)] = &[
    ...
    (
        "unlink-window",
        "unlinkw",
        "Unlink a window from the session",
    ),
    // Pane
    ...
    (
        "capture-pane",
        "capturep",
        "Capture pane contents to buffer",
    ),
...
pub fn build_overlay_lines(
    ...
    let prefix_bindings: Vec<_> = user_bindings
        .iter()
    ...
    let non_prefix: Vec<_> = user_bindings
        .iter()
        .filter(|(_, t, _, _)| t != "prefix")
        .collect();
