...
fn thai_and_cjk_widths() {
    ...
    assert_eq!(
        cursor_col_after("\u{0E01}"),
        1,
        "base consonant should be 1 cell"
    );

    // base + MAI EK tone mark (Mn, width 0) -> still 1 cell
    assert_eq!(
        cursor_col_after("\u{0E01}\u{0E48}"),
        1,
        "base + tone mark should be 1 cell"
    );

    // base + SARA I above vowel (Mn) -> 1 cell
    assert_eq!(
        cursor_col_after("\u{0E01}\u{0E34}"),
        1,
        "base + above vowel should be 1 cell"
    );

    // base + MAI HAN-AKAT U+0E31 (Mn) -> 1 cell
    assert_eq!(
        cursor_col_after("\u{0E01}\u{0E31}"),
        1,
        "base + hanakat should be 1 cell"
    );

    // full repro word "ก่อน" = 0E01 0E48 0E2D 0E19 -> 3 base cells
    assert_eq!(
        cursor_col_after("\u{0E01}\u{0E48}\u{0E2D}\u{0E19}"),
        3,
        "kaawn should be 3 cells"
    );

    // Arabic base + fatha (Mn) -> 1 cell
    assert_eq!(
        cursor_col_after("\u{0627}\u{064E}"),
        1,
        "arabic base + fatha should be 1 cell"
    );

    // CJK ideograph U+4E00 (wide) -> 2 cells
    assert_eq!(
        cursor_col_after("\u{4E00}"),
        2,
        "CJK ideograph should be 2 cells"
    );
...
fn full_repro_string_width() {
    ...
    assert_eq!(
        cursor_col_after(s),
        19,
        "full repro string should be 19 cells"
    );
}
...
fn per_cluster_widths() {
    let cases: &[(&str, u16)] = &[
        (
            "\u{0E1B}\u{0E0F}\u{0E34}\u{0E1A}\u{0E31}\u{0E15}\u{0E34}\u{0E01}\u{0E32}\u{0E23}",
            7,
        ), // ปฏิบัติการ (7 spacing + 3 marks)
